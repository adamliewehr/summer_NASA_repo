...
CHIP_HALF_SIZE = 256                        # 512x512 chips
...
def get_abi_chip_at_time(t, yy, ddn, coords, ABI_ROOT, angle=0.0):
    ...
    # Expanded extraction to avoid black corners after rotation
    EXTRACTION_HALF_SIZE = math.ceil(CHIP_HALF_SIZE * 1.414)

    chip = abi[
        coords[0] - EXTRACTION_HALF_SIZE:coords[0] + EXTRACTION_HALF_SIZE,
        coords[1] - EXTRACTION_HALF_SIZE:coords[1] + EXTRACTION_HALF_SIZE,
        :
    ]

    # Rotate the chip so the track is vertical
    if angle != 0:
        chip = scipy.ndimage.rotate(chip, -angle, reshape=False, order=1)

    # Center crop back to CHIP_HALF_SIZE
    start_h = EXTRACTION_HALF_SIZE - CHIP_HALF_SIZE
    start_w = EXTRACTION_HALF_SIZE - CHIP_HALF_SIZE
    chip = chip[
        start_h:start_h + 2 * CHIP_HALF_SIZE,
        start_w:start_w + 2 * CHIP_HALF_SIZE,
        :
    ]
    ...
    for dt_min in offsets_minutes:
        try:
            ...
            chip, hour_str, minute_str = get_abi_chip_at_time(
                t_t, yy_t, ddn_t, coords, ABI_ROOT, angle=angle
            )
            valid = 1
    ...
    while i < N - half_p:
        try:
            ...
            try:
                ...
                row_diff = coord_end[0] - coord_start[0]
                col_diff = coord_end[1] - coord_start[1]
                track_angle = math.degrees(math.atan2(col_diff, row_diff))
            except Exception:
                track_angle = 0.0
